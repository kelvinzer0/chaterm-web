...
app.post('/api/db/assets/children', (req, res) =>
  res.json({ ok: true, databases: ['main'], tables: ['users', 'db_groups', 'db_assets'], objects: [], columns: ['id'] })
)
...
app.post('/api/db/assets/query', (req, res) =>
  res.json({ ok: true, columns: ['status'], rows: [{ status: 'Query OK' }], rowCount: 1, durationMs: 1 })
)
...
app.post('/api/db/assets/query-table', (req, res) =>
  res.json({ ok: true, columns: ['id'], rows: [{ id: 1 }], rowCount: 1, durationMs: 1, total: 1, knownColumns: ['id'] })
)
...
app.post('/api/credentials/create', (req, res) => {
  ...
  db.prepare(
    'INSERT INTO credentials (key_chain_id, name, chain_type, username, password, private_key, passphrase) VALUES (?, ?, ?, ?, ?, ?, ?)'
  ).run(id, name, chain_type || 'PASSWORD', username || '', password || '', private_key || '', passphrase || '')
